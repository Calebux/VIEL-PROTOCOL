...
/**
 * Create a new Veil note with random nullifier and secret.
 */
export async function createNote(amount: bigint): Promise<VeilNote> {
 ...
 * Format: veil-<hex(nullifier)>-<hex(secret)>-<amount>-<leafIndex>
 */
export function serializeNote(note: VeilNote): string {
  const parts = [
    "veil",
 ...
 */
export async function deserializeNote(encoded: string): Promise<VeilNote> {
