...
async function main() {
  console.log("╔════════════════════════════════════════════════╗");
  console.log("║   Veil Protocol — End-to-End Demo             ║");
  console.log("╚════════════════════════════════════════════════╝\n");
  ...
  if (deployerSecret) {
    ...
    const newPoolId = execSync(
      `stellar contract deploy --wasm contracts/target/wasm32v1-none/release/veil_pool.wasm --source "${deployerSecret}" ${CLI_FLAGS}`,
      { encoding: "utf-8", cwd, timeout: 60000 }
    ).trim();
  ...
  // Serialize the note
  const noteString = `veil-${nullifier.toString(16)}-${secret.toString(16)}-1000000000-${leafIndex}`;
  console.log(`\n  ┌─────────────────────────────────────────────────┐`);
