...
async function main() {
  console.log("=== Veil Protocol — Contract Initialization ===\n");

  const deployerAddr = run("stellar keys address veil-deployer");
  ...
  try {
    const result = run(
      `stellar contract invoke --id ${VERIFIER_ID} --source veil-deployer --network testnet -- initialize --admin ${deployerAddr} --vk '${vkJson}'`
    );
  ...
  console.log("\n[2/2] Initializing Veil Pool...");
  try {
    const result = run(
      `stellar contract invoke --id ${POOL_ID} --source veil-deployer --network testnet -- initialize --admin ${deployerAddr} --token ${TOKEN_ID} --verifier ${VERIFIER_ID} --denomination ${DENOMINATION}`
    );
  ...
  // Write .env
  const deployerSecret = run("stellar keys show veil-deployer");
  console.log("\nWriting .env...");
